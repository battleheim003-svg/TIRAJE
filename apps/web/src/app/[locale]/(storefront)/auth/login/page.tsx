  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      try {
        ...
        if (!result.success) {
          setError(result.error ?? (fa ? "ورود ناموفق بود. اطلاعات خود را بررسی کنید." : "Login failed. Please check your credentials."))
          return
        }
      ...
      <div className="auth-root">
        <div className="auth-card">
          <div className="auth-head">
            <h1 className="auth-title">{fa ? "ورود به حساب کاربری" : "Sign In"}</h1>
            <p className="auth-sub">
              {fa ? "حساب ندارید؟" : "No account?"}{" "}
              <Link href={`/${locale}/auth/register`} className="auth-link">
          ...
          {justRegistered && (
            <p className="auth-success" role="status">
              {fa ? "ثبت‌نام موفق! لطفاً وارد شوید." : "Registration successful! Please sign in."}
            </p>
          )}
